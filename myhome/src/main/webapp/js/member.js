...
// 사용자의 입력값 3개 = > tr > td * 3 => tbody 하위요소 추가.
 ...
 document.querySelector('#addMember').addEventListener('click', addMemberFnc) ;
   function addMemberFnc(){
      let mem = {
         NoValue: document.querySelector('#memberNo').value,
         NameValue: document.querySelector('#memberName').value,
         PointValue: document.querySelector('#memberPoint').value      
      }
      let tr = makeRow(mem);
...
document.querySelector('#modifyMember').addEventListener('click',modifyMemberFnc);
   function modifyMemberFnc(){
      let NoValue = document.querySelector('#memberNo').value;
      document.querySelectorAll('table#tlist tbody tr').forEach(function(tr){
         if (tr.children[0].innerText === NoValue) {
            tr.children[1].innerText = document.querySelector('#memberName').value,
            tr.children[2].innerText = document.querySelector('#memberPoint').value      
         };
